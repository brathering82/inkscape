...
class ScaleGen(inkex.Effect):
	...
	def __init__(self):
		...
		self.OptionParser.add_option('', '--perplinestrokewidth', action = 'store',
			type = 'float', dest = 'perplinestrokewidth', default = '1',
	...
	def addLabel(self, n, x, y, grp, fontsize, phi = 0.0):
	...
	def addLine(self, i, scalefrom, scaleto, grp, grpLabel, type=2):
		...
		#label line
		if type==0:
			line_style = { 'stroke': 'black',	'stroke-width': labellinestrokewidth }
			x1 = 0
			y1 = i*res
		...
		#long line
		if type==1:
			line_style = { 'stroke': 'black', 'stroke-width': longlinestrokewidth }
			x1 = 0
			y1 = i*res
		...
		#short line
		if type==2:
			line_style = { 'stroke': 'black', 'stroke-width': shortlinestrokewidth }
			x1 = 0
			y1 = i*res
		...
		#perpendicular line
		if type==3:
			line_style = { 'stroke': 'black', 'stroke-width': perplinestrokewidth }			
	...
	def addLineRad(self, i, scalefrom, scaleto, grp, grpLabel, type=2, ishorizontal=True):
		...
		if type==3:		
			...
			line_style = {'stroke': 'black', 'stroke-width' : perplinestrokewidth, 'fill': 'none'}			
	...
	def skipfunc(self, i, markArray, grpMarkArray):
	
		skip = True
		grpMark = grpMarkArray[0]
		type = 0
		...
		if markArray[1] != 0 and skip==1:
			if (i % markArray[1])==0:
				...
				grpMark = grpMarkArray[1]
				skip = False
				
		if markArray[2] != 0 and skip==1:
			if (i % markArray[2])==0:
				type = 2 	# the short line
				grpMark = grpMarkArray[2]
				skip = False
				
		return (skip, grpMark, type)
	...
	def effect(self):
		...
		grp_attribs = {inkex.addNS('label','inkscape'):grp_name,
						'transform':grp_transform }
		grpMarkArray[0] = inkex.etree.SubElement(self.current_layer, 'g', grp_attribs)
		grp_name = 'Long line'
		grp_attribs = {inkex.addNS('label','inkscape'):grp_name,
						'transform':grp_transform }
		grpMarkArray[1] = inkex.etree.SubElement(self.current_layer, 'g', grp_attribs)	
		grp_name = 'Short line'
		grp_attribs = {inkex.addNS('label','inkscape'):grp_name,
						'transform':grp_transform }
		grpMarkArray[2] = inkex.etree.SubElement(self.current_layer, 'g', grp_attribs)	
		...
		if scaletype == 'straight':
			...
			for i in range(scalefrom, scaleto):	
				skip, grpMark, type = self.skipfunc(i, markArray, grpMarkArray)				
				if skip==False:
					self.addLine(i, scalefrom, scaleto, grpMark, grpLabel, type) # addLabel is called from inside

			#add the perpendicular line
			if perpline==True:
				self.addLine(0, scalefrom, scaleto, grpMarkArray[0], grpLabel, 3)		
			...
			for i in range(scalefrom, scaleto):
				skip, grpMark, type = self.skipfunc(i, markArray, grpMarkArray)      		
				if skip==False:	
					self.addLineRad(i, scalefrom, scaleto, grpMark, grpLabel, type, ishorizontal) # addLabel is called from inside
				
			#add the perpendicular (circular) line
			if perpline==True:
				self.addLineRad(0, scalefrom, scaleto, grpMarkArray[0], grpLabel, 3, ishorizontal)
